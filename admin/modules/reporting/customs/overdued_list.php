...
<?php
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        $_buffer .= '<div style="font-size: 10pt; margin-bottom: 3px;"><div id="' . $array_data[0] . 'emailStatus"></div>' . __('E-mail') . ': <a href="mailto:' . $member_d[1] . '">' . $member_d[1] . '</a> - <a class="usingAJAX" href="' . MWB . 'membership/overdue_mail.php' . '" postdata="memberID=' . $array_data[0] . '" loadcontainer="' . $array_data[0] . 'emailStatus">' . __('Send Notification e-mail') . '</a> - ' . __('Phone Number') . ': <a href="https://wa.me/' . $member_d[2] . '?text=Pemberitahuan bahwa anda memiliki pinjaman buku di UPT Pusat Perpustakaan IAIN Padangsidimpuan sejumah Mohon untuk segera dikembalikan atau diperpanjang masa pinjam sebelum jatuh tempo. Terima kasih "target="_blank">' . $member_d[2] . '</a></div>';
