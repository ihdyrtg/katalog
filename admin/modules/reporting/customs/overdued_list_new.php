...
<?php
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        while ($ovd_title_d = $ovd_title_q->fetch_assoc()) {
            ...
            $_buffer .= '<tr>';
            $_buffer .= '<div style="font-size: 10pt; margin-bottom: 3px;"><div id="' . $array_data[0] . 'emailStatus"></div>' . __('E-mail') . ': <a href="mailto:' . $member_d[1] . '">' . $member_d[1] . '</a> - <a class="usingAJAX" href="' . MWB . 'membership/overdue_mail.php' . '" postdata="memberID=' . $array_data[0] . '" loadcontainer="' . $array_data[0] . 'emailStatus">' . __('Send Notification e-mail') . '</a> - ' . __('Phone Number') . ': Kirim Notifikasi WA <a href="https://wa.me/' . $member_d[2] . '?text=Pemberitahuan bahwa *' . $member_d[0] . ',* ada pinjaman buku dengan keterlambatan *' . $ovd_title_d['Overdue Days'] . '  hari dengan denda Rp. 1000 per hari* di Perpustakaan UIN SYAHADA dengan Kode Barcode: *' . $ovd_title_d['item_code'] . '*, Judul: *' . $ovd_title_d['title'] . '*. Terima Kasih. ===Perpustakaan UIN SYAHADA=== "target="_blank"> ' . $member_d[2] . '' . $ovd_title_d[3] . '</a></div>';
